...
docker build --build-arg GOCD_VERSION=${_gocd_version} --tag "atifsaddique/gocd-server:${_gocd_tag}"  --no-cache=true .

if [ $_release_build == true ]; then
	docker tag "stakater/gocd-server:${_gocd_tag}" "atifsaddique/gocd-server:latest"
fi